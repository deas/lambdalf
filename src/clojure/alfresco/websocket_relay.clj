...
(comment defn- send-json-over-ws
         [channel data]
         (let [json-data (json/generate-string data)]
           #_(println json-data)
           (cr-nrepl/send! channel json-data)))

(comment defn- process-message
  [channel data]
...
(comment defn ring-handler
         ...
         [request]
         (cr-nrepl/with-channel
           request
           channel
           (cr-nrepl/on-receive channel (partial process-message channel))))