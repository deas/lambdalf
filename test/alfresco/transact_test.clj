...
(deftest transact-itests
  ...
  (testing "Obtaining company home as admin in tx failed."
    (is (= "Company Home" (t/in-ro-tx-as (a/admin)
                                         (n/property (n/company-home)
                                                     :cm/name)))))
  (testing "Obtaining share site ad admin in tx failed"
    (is (= true (t/in-ro-tx-as
                 (a/admin)
                 (n/site? (first (n/children (n/sites-home)))))))))
